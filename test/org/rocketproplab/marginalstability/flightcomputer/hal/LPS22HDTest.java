...
public class LPS22HDTest {
  ...
  @Test
  public void getSamplableReturnsSampleableSensor() {
    MockI2CDevice           i2c       = new MockI2CDevice();
    BarometerTime           time      = new BarometerTime();
    LPS22HD                 barometer = new LPS22HD(i2c, time);
    SamplableSensor<Double> samplable = barometer.getSamplable();
    ...
    assertTrue(samplable.hasNewData());
    assertEquals(1260, samplable.getNewData(), 0.0005);
    assertEquals(-3.2, samplable.getLastSampleTime(), 0.0005);
    assertFalse(samplable.hasNewData());
    ...
    assertTrue(samplable.hasNewData());
    assertEquals(1260, samplable.getNewData(), 0.0005);
    assertEquals(10, samplable.getLastSampleTime(), 0.0005);
  }
